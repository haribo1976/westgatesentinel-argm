...
| D3 Value Alignment | CLAUDE.md D3 directive, task classification | .cursorrules priority rules | AGENTS.md priority section | AGENTS.md priority section | GEMINI.md priority section | config priority rules | AGENTS.md priority section | Modelfile SYSTEM block |
| D4 Infrastructure Governance | CLAUDE.md D4 directive | .cursorrules scope limits | AGENTS.md scope section | AGENTS.md scope section | GEMINI.md scope section | config scope rules | AGENTS.md scope section | Modelfile SYSTEM block |
| D5 Operational Efficiency | CLAUDE.md D5 directive, JSON output schema | .cursorrules output format | AGENTS.md output section | AGENTS.md output section | GEMINI.md output section | config output rules | AGENTS.md output section | Modelfile SYSTEM block |
| D6 Delivery Standards | CLAUDE.md D6 directive, template references | .cursorrules format rules | AGENTS.md format section | AGENTS.md format section | GEMINI.md format section | config format rules | AGENTS.md format section | Modelfile SYSTEM block |
...
> Scope permissions explicitly using `--allowedTools` or permission mode flags. Never bypass the permission system.
...
> **Note:** The `amp.config.yaml` governance schema shown below is illustrative. Verify Amp's current configuration support before deployment.
...
> **Note:** Droid CLI governance support has not been independently verified. Treat this section as provisional.
...
At ARGM Level 5, the same directive framework must apply to all agents, not the primary agent only. The following controls enforce cross-agent consistency:
