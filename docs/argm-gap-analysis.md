...
| **Swarm governance** | AISM | Multi-agent swarm coordination, agent-to-agent trust, distributed governance. ARGM requires cross-agent consistency (Level 4) but does not govern inter-agent communication. |
| **Threat model specificity** | AISM | Agent Threat and Control Matrix with 10 threat categories, MITRE ATLAS cross-references, OWASP LLM mappings. ARGM defers to existing threat frameworks. |
| **Immutable audit trails** | AISM | Sovereignty level requires immutable ledgers and cryptographic logging. ARGM requires logging but does not specify immutability. |
| **Compliance crosswalk granularity** | AISM | Control-level mapping to NIST AI RMF, ISO 42001. ARGM provides level-to-framework mapping, not control-level. |
...
3. Cost governance for agentic operations
4. Autonomous operation safety controls (turn limits, dry-run defaults, overnight rules)
5. Governance self-monitoring and drift detection
6. Progressive maturity with specific observable evidence at each level
7. Conflict resolution order as a governance primitive
